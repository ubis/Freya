...
// Returns client remote end point
func (c *Client) GetEndPnt() string {
	return c.endpnt
}
	...
	for err == nil {
		...
		if req.Method != "" {
			...
			if err = c.readRequest(&req); err != nil {
				log.Error("rpc: error reading request:", err.Error())
			}
		} else {
			// response comes to client
			if err = c.readResponse(&resp); err != nil {
				log.Error("rpc: error reading response:", err.Error())
			}
		}
	}
...
func (c *Client) readRequest(req *Request) error {
	...
	if !ok {
		resp := &Response{
			Seq:   req.Seq,
			Error: "rpc: can't find method " + req.Method,
		}
